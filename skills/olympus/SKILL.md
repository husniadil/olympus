...
- Servers are the level above sessions. `olympus servers` lists them (tmux socket names, herdr named sessions, zmx's one directory) and `--server <name>` points any verb at one by name; on meja neither exists. `olympus servers stop <name>` takes every session on it down, so use it only when the user means the whole server. `olympus servers start [name]` brings one up without creating anything on it, which is what a machine that has just rebooted needs: herdr restores the panes that session was running when its server boots, and no other verb will boot one.

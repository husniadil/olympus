...
- `olympus agents` lists the coding agents running in panes, on every backend. `detected_by` says how each was found: `herdr` rows carry `status` (working, idle), `title` and `usage`; `command` rows were matched on a known agent's name in the pane's process tree (or its foreground command where the pane has no `pid`) and their status is `unknown`, so read the pane's `screen` to learn more. `olympus capabilities` reports `agent_status` where rows can carry one.

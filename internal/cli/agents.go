...
func (a *App) agentsCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long: "List the coding agents running in panes on the resolved backend: which pane, which agent, what it is working on, and its status — working, idle, blocked — where it can be told." +
			"\n\nEvery backend answers. How a row was found is disclosed in detected_by: herdr detects agents itself and reports status, title and usage; on the other backends an agent is a pane whose process tree holds a known agent (claude, codex, gemini, aider, opencode, goose, amp, cursor, pi, omp, copilot, devin, agy, cline, droid, kimi, kiro, kilo, hermes, qodercli, qwen, mastracode, maki, muse, grok) — its foreground command where the pane has no pid — and its status is read off a capture of the pane by the agent's manifest, one capture per row. status_source says which: native or screen. What no rule recognises is unknown rather than guessed. `olympus capabilities` reports agent_status where rows can carry a status." +
			scriptsNote,
		Args: cobra.NoArgs,

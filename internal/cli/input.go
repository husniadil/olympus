...
func (a *App) sendCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Deliver text, confirm it is on screen, and only then submit it." +
			"\n\nA target whose agent is waiting on a person — a permission prompt, a question — is refused with AGENT_BLOCKED (exit 8) and nothing is typed, since the Enter would answer the prompt. Answer it with press. Where the agent draws an input box, the text is looked for inside that box rather than anywhere on the screen." +
